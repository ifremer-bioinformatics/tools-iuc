...
format_data_pie <- function(PHYLOSEQ, env_var, taxa, unknown, taxa_nb) {
  ...
  table_top <- ordered_abund[1:taxa_nb,]
  total_tax <- length(table_top[, taxa][!is.na(table_top[, taxa])])
  if ( as.numeric(total_tax) < as.numeric(taxa_nb) ) {
    numb_taxa <- total_tax
    round_perc_min_relabund_value <- "NA"
  } else {
    numb_taxa <- taxa_nb
    min_abund_value_top <- tail(table_top$Abundance,1)
    total_abund_value <- sum(ordered_abund$Abundance)
    min_relabund_value <- min_abund_value_top / total_abund_value
    perc_min_relabund_value <- min_relabund_value * 100
    if ( as.numeric(perc_min_relabund_value) > 0.005 ) {
      round_perc_min_relabund_value <- round(perc_min_relabund_value, 2)
    } else {
      round_perc_min_relabund_value <- round(perc_min_relabund_value, 3)
    }
  }
  list_ordered_abund <- as.character(ordered_abund[, taxa])
  list_ordered_abund <- list_ordered_abund[list_ordered_abund != unknown]
  top <- list_ordered_abund[1:numb_taxa]
  mdf[, taxa] <- as.character(mdf[ , taxa])
  ii <- (mdf[, taxa] %in% c(top, unknown))
  Others_lab <- paste("Others (", taxa, " <", round_perc_min_relabund_value, "%)", sep="")
...
ggplot_pie <- function(PHYLOSEQ, env_var, taxa, unknown, taxa_nb, color_pie, pie_data_filename) {
  ...
  list_taxa = as.character(unique(formated_data[, taxa]))
  if (any(list_taxa %in% Others_lab) & any(list_taxa %in% unknown)) {
  ...
  } else if(any(list_taxa %in% Others_lab)) {
    taxa4color <- as.character(unique(formated_data[, taxa]))
    taxa4color <- taxa4color[ ! taxa4color %in% Others_lab]
    colvals <- c(color_pie[1:taxa_nb], "grey45")
    names(colvals) <- c(taxa4color, Others_lab)
  } else if(any(list_taxa %in% unknown)) {
    taxa4color <- as.character(unique(formated_data[, taxa]))
    taxa4color <- taxa4color[ ! taxa4color %in% unknown]
    colvals <- c(color_pie[1:taxa_nb], "black")
    names(colvals) <- c(taxa4color, unknown)
  } else {
    taxa4color <- as.character(unique(formated_data[, taxa]))
...
ggplot_barplot <- function(PHYLOSEQ, env_var, taxa, unknown, taxa_nb, color_bar, abundance_data_filename) {
  ...
  if (any(list_taxa %in% Others_lab) & any(list_taxa %in% unknown)) {
    ...
    taxa4color <- as.character(unique(formated_data[, taxa]))
    taxa4color <- taxa4color[ ! taxa4color %in% Others_lab]
    colvals <- c(color_bar[1:taxa_nb], "grey45")
    names(colvals) <- c(taxa4color, Others_lab)
  } else if(any(list_taxa %in% unknown)) {
    taxa4color <- as.character(unique(formated_data[, taxa]))
    taxa4color <- taxa4color[ ! taxa4color %in% unknown]
    colvals <- c(color_bar[1:taxa_nb], "black")
    names(colvals) <- c(taxa4color, unknown)
  } else {
    taxa4color <- as.character(unique(formated_data[, taxa]))
...
df_alpha_div_sdf = data.frame(df_alpha_div, sample_data(PHYLOSEQ), check.names=FALSE)
...
df_alpha_div_sdf[, args[3]] = as.factor(df_alpha_div_sdf[, args[3]])
...
stats_var = df_alpha_div_sdf %>% group_by(Measure) %>% tukey_hsd(as.formula(test_formula)) %>% adjust_pvalue(method="bonferroni") %>% add_significance() %>% add_y_position(scales="free")
...
metadata_df$Samples = as.factor(rownames(metadata_df))
rarefaction_curve = rarecurve(asv_matrix, step=20, tidy=TRUE)
colnames(rarefaction_curve) = c("Samples", "Iter", "Richness")
rarefaction_curve_metadata = left_join(rarefaction_curve, metadata_df, by="Samples")

rarefaction_value = min(rowSums(asv_matrix))

FINAL_rarefaction_curve = ggplot(rarefaction_curve_metadata) +
  geom_line(aes_string(x="Iter", y="Richness", group="Samples", colour=args[3])) +
  theme_bw() +
...
if(args[7] == "pr2-4" || args[7] == "pr2-5" || args[7] == "MARE-MAGE") {
    unknown = "Unknown Eukaryota"
}

if(args[7] == "MARE-MAGE") {
  ggplot_pie(PHYLOSEQ, args[3], "Kingdom", unknown, args[6], color_set)
  ggplot_pie(PHYLOSEQ, args[3], "Clade", unknown, args[6], color_set)
}
if(args[7] == "silva" || args[7] == "MARE-MAGE") {
...
if(args[7] == "MARE-MAGE") {
  ggplot_barplot(PHYLOSEQ, args[3], "Kingdom", unknown, args[6], color_set)
  ggplot_barplot(PHYLOSEQ, args[3], "Clade", unknown, args[6], color_set)
}
if(args[7] == "silva" || args[7] == "MARE-MAGE") {
